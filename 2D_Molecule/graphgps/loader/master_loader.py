...
# from graphgps.transform.dist_transforms import (  # effective_resistance_embedding,; effective_resistances,; effective_resistances_from_embedding,
#     add_dist_features,
#     add_reverse_edges,
#     add_self_loops,
# )
...
# from graphgps.transform.transforms import (  # not the same as split_generator above.; clip_graphs_to_size,; concat_x_and_pos,; move_node_feat_to_x,; pre_transform_in_memory,; typecast_x,
#     generate_splits,
#     move_node_feat_to_x,
...
#     # Inductive graph-level split (there is no train/test edge split).
#     s_dict = dataset.get_idx_split(split_name)
#     # of training: 476490; after sample: 47649
#     dataset.split_idxs = [s_dict[s] for s in ["train", "val", "test"]]
#     if cfg.dataset.resample_negative:
#         dataset.transform = structured_neg_sampling_transform
#     return dataset
...
#         )
#         raise e
#     # if cfg.gnn.num_vns == 0:
#     #     transforms = None
#     # else:
#     #     transforms = T.Compose([VirtualNode() for _ in range(cfg.gnn.num_vns)])
#     dataset_type = name.split("-", 1)[1]
#     if dataset_type == "functional":
#         dataset = PeptidesFunctionalDataset(dataset_dir)  # , pre_transform=transforms
...
from graphgps.loader.split_generator import prepare_splits, set_dataset_splits
from graphgps.transform.posenc_stats import compute_posenc_stats
from graphgps.transform.transforms import (clip_graphs_to_size,
                                           concat_x_and_pos,
...
def log_loaded_dataset(dataset, format, name):
    ...
    logging.info(f"  undirected: {dataset[0].is_undirected()}")
    logging.info(f"  num graphs: {len(dataset)}")
    # * num of training graph
    logging.info(f"  num training graphs: {len(dataset.split_idxs[0])}")
...
def preformat_OGB_Graph(dataset_dir, name):
    ...
    if name == "ogbg-ppa":
        ...
        from graphgps.loader.ogbg_code2_utils import (augment_edge,
                                                      encode_y_to_arr,
                                                      get_vocab_mapping,
                                                      idx2vocab)
...
def preformat_PCQM4Mv2Contact(dataset_dir, name):
    ...
    try:
        # Load locally to avoid RDKit dependency until necessary
        from graphgps.loader.dataset.pcqm4mv2_contact import (
            PygPCQM4Mv2ContactDataset, structured_neg_sampling_transform)
    except Exception as e:
        logging.error(
            "ERROR: Failed to import PygPCQM4Mv2ContactDataset, "
            "make sure RDKit is installed."
